...
import type { NewsArticle, NewsFormData } from "./types.js";
...
async function editNews(id: number) {
  ...
  if (authorIdEl) authorIdEl.value = String(article.authorId);
...
function populateAuthorSelect() {
  ...
  if (!(authorSelect instanceof HTMLSelectElement)) return;
